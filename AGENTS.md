...
- Cross-package types live in `shared/` and import via the `@shared/*` alias. Never duplicate a type that already exists in `shared/`.
...
- Co-locate component-specific helpers; promote to `shared/` only when used across packages.

**Backend / Sockets**
- Keep `SocketManager` thin: it routes events and delegates to model methods. Game logic lives in `backend/models/**` and `backend/logic/**`, not in the socket handler.
- Every emit that mutates room state should go through a single canonical event (e.g., `"room updated"`) carrying the full serialized room — clients should not reconstruct state from partial diffs.
- Validate inbound socket payloads. Treat the client as untrusted.
...
- Commit messages: imperative mood, lowercase prefix matching the branch (`feat:`, `fix:`, `chore:`, `docs:`). Body explains *why* if non-obvious.
